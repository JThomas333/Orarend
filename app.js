import express from "express";
import { initializeDatabase, dbAll, dbRun } from "./util/database.js";
import cors from "cors"

const app = express();
app.use(cors())
app.use(express.json());

app.get("/timetable",async (req, res) => {
    const timetable = await dbAll("SELECT * FROM timetable");
    res.json(timetable);
});

app.post("/timetable", async (req, res) => {
    const { day, ora1, ora2, ora3, ora4, ora5, ora6, ora7, ora8, ora9 } = req.body
    const timetable = await dbRun(`INSERT INTO timetable 
        (day, ora1, ora2, ora3, ora4, ora5, ora6, ora7, ora8, ora9) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`, [day, ora1, ora2, ora3, ora4, ora5, ora6, ora7, ora8, ora9])
        res.sendStatus(200)
});

app.put("/timetable/:id", async (req, res) => {
    const { day, ora1, ora2, ora3, ora4, ora5, ora6, ora7, ora8, ora9 } = req.body
    await dbRun(
        `UPDATE timetable SET day=?, ora1=?, ora2=?, ora3=?, ora4=?, ora5=?, ora6=?, ora7=?, ora8=?, ora9=? WHERE id=?`,
        [day, ora1, ora2, ora3, ora4, ora5, ora6, ora7, ora8, ora9, req.params.id]
    )
    res.sendStatus(200)
});
app.delete("/timetable/:id", async (req, res) => {
    await dbRun("DELETE FROM timetable WHERE id = ?", [req.params.id]);
    res.sendStatus(200)
});

async function startServer() {
    await initializeDatabase();
    app.listen(3000, () => {
      console.log("Server runs on port 3000");
    });
  }
  
  startServer();